// app\admin\dashboard\components\ActivityModal.tsx
'use client'
...
export default function ActivityModal({
  ...
  // Reset preview when modal opens with new activity
  useEffect(() => {
    if (isOpen) {
      ...
      // Log untuk debugging
      console.log('🔄 ActivityModal opened:', {
        mode,
        hasCurrentActivity: !!currentActivity,
        hasImage: !!currentActivity?.image_url,
        imageUrl: currentActivity?.image_url?.substring(0, 100) + '...',
        imageType: currentActivity?.image_url?.startsWith('http') ? 'URL' : 
                  currentActivity?.image_url?.startsWith('data:') ? 'BASE64' : 
                  'OTHER/EMPTY'
      })
    }
  }, [isOpen, currentActivity, mode])

  // Handle main image change dengan validasi
  const handleMainImageChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (file.size > MAX_SIZE) {
        setImageError(`❌ File size too large (${(file.size / 1024 / 1024).toFixed(2)}MB). Maximum is 2MB.`)
      ...
      if (!validTypes.includes(file.type)) {
        setImageError('❌ Invalid file type. Please select JPEG, PNG, WEBP, or GIF image.')
        e.target.value = ''
        setFileName('')
        return
      }
      ...
      if (!fileExt || !validExtensions.includes(fileExt)) {
        setImageError(`❌ Invalid file extension (.${fileExt}). Allowed: ${validExtensions.join(', ')}`)
        e.target.value = ''
        setFileName('')
        return
      }
      
      // Preview image (temporary base64 untuk preview saja)
      ...
      reader.onloadend = () => {
        ...
        // Hanya untuk preview di modal, tidak disimpan ke database
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
      <div className="bg-gray-800 rounded-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-gray-700 shadow-2xl">
        ...
        <form onSubmit={handleSubmit}>
          <input type="hidden" name="id" defaultValue={currentActivity?.id || ''} />
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-gray-900 border border-gray-700 rounded-lg p-3 h-36 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition duration-200 resize-none"
                required
              ...
              <div className="flex items-center gap-1 mt-2 text-xs text-gray-400">
                <MdInfo className="w-3 h-3" />
                <span>Minimal 10 karakter, maksimal 500 karakter</span>
              </div>
            </div>
            ...
            <div className="bg-gray-900/50 rounded-lg p-5 border border-gray-700">
              ...
              {imageError && (
                <div className="mb-4 p-3 bg-red-900/30 border border-red-700/50 rounded-lg flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-red-300 text-sm font-medium">Image Error</p>
              ...
              {fileName && !imageError && (
                <div className="mb-4 p-3 bg-green-900/20 border border-green-700/30 rounded-lg flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-green-400 text-xs truncate" title={fileName}>
                      {fileName}
                    </p>
                  </div>
                  <button
              ...
              <div className="mb-5">
                ...
                <div className={`relative w-full h-64 rounded-lg overflow-hidden border-2 ${
                  mainImagePreview ? 'border-gray-600' : 'border-dashed border-gray-700'
                }`}>
                  {mainImagePreview ? (
                    <>
                      <img 
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-gray-800 to-gray-900 flex flex-col items-center justify-center">
                      ...
                      <p className="text-gray-500 text-xs mt-1">Upload a thumbnail image</p>
                    </div>
                  )}
                </div>
                ...
                {currentActivity?.image_url && !fileName && (
                  <div className="mt-2 text-xs text-gray-500">
                    ...
                    <span className="truncate max-w-xs inline-block align-bottom" title={currentActivity.image_url}>
                ...
                <div className="bg-gray-950/50 p-4 rounded-lg border border-gray-800">
                  ...
                  <ul className="text-xs text-gray-400 space-y-1.5">
                    <li className="flex items-start gap-2">
                      ...
                      <span>Upload thumbnail untuk activity (wajib untuk activity baru)</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-primary mt-0.5">•</span>
                    ...
                    {mode === 'edit' && (
                      <li className="flex items-start gap-2">
                        ...
                        <span className="text-yellow-400">Kosongkan jika tidak ingin mengganti thumbnail</span>
                      </li>
                    )}
                    <li className="flex items-start gap-2">
                      <span className="text-primary mt-0.5">•</span>
                      <span className="text-gray-500">Gambar akan disimpan di Supabase Storage, bukan di database</span>
                    </li>
                  </ul>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-1 mt-2 text-xs text-gray-400">
                  <MdInfo className="w-3 h-3" />
                  <span>Angka lebih kecil akan muncul lebih dulu (0 = pertama)</span>
                </div>
              </div>
            </div>
          </div>
